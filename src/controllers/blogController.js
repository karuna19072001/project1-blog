...
const getBlogs = async function (req, res) {
    ...
    try {
        ...
        let availableBlogs = await blogModel.find({ $and: [filters, { isDeleted: false }, { isPublished: true }] }).populate("authorId")

        if (availableBlogs.length == 0) {
            return res.status(404).send({ status: false, msg: "No Book Found For Given info" })
        }
        return res.status(200).send({ status: true, msg: availableBlogs })
